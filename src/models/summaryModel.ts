...
async function initializeUSSummary() {
    // clear old data
    await summaryModel.deleteMany({})
    // get population by state
    var life_span_api = "https://api.census.gov/data/2021/pep/population?get=NAME,POP_2021&for=us:*";
    var docs: any = await fetch(life_span_api).then(result => result.json());
...
async function getUSSummary() {
    return summaryModel.find({}).then((document) => {
